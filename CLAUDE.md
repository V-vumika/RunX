...
**Real stack (what's installed):** Next.js 16 (App Router) + TS, Tailwind v4 + shadcn/ui (Radix), Zustand, Pyodide 0.28.3 (loaded from CDN, runs in a classic Web Worker), Monaco, Framer Motion, plain-SVG tree/graph visualizers (React Flow and d3 were removed 2026-07-13 — zero imports), **acorn + astring** (JS parse/regenerate for the tracer and entry detection — approved 2026-07-12, no further JS-parsing deps needed). Later: a backend sandbox for Java/C++ (Docker + GDB-MI / source instrumentation), Supabase, OpenAI/Gemini.
