...
/** A single shimmering pixel square on the canvas. */
class Pixel {
  width: number;
  height: number;
...
/**
 * Registers the <pixel-canvas> custom element, lazily and in the browser only.
 * Defining a class that `extends HTMLElement` at module scope would crash SSR
 * (HTMLElement is undefined in Node), so the definition lives here and runs from
 * the React wrapper's effect.
 */
...
function registerPixelCanvas() {
  ...
  class PixelCanvasElement extends HTMLElement {
    ...
    private _onAppear = () => this.handleAnimation("appear");
    private _onDisappear = () => this.handleAnimation("disappear");
...
const PixelCanvas = React.forwardRef<HTMLDivElement, PixelCanvasProps>(
  ({ gap, speed, colors, variant, noFocus, style, ...props }, ref) => {
    ...
    return React.createElement("pixel-canvas" as unknown as string, {
      ref,
      "data-gap": gap,
      "data-speed": speed,
