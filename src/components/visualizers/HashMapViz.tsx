...
function NodeCard({ node, showConnector }: { node: HNode; showConnector: boolean }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: deleted ? 0.55 : 1, scale: 1, y: 0 }}
      exit={{ opacity: 0, scale: 0.6, y: 8 }}
      transition={{ type: "spring", stiffness: 300, damping: 24 }}
      className="flex w-full flex-col items-center gap-1"
    >
      {showConnector && <span className="text-[10px] leading-none text-white/30">↓</span>}
      <motion.div
        animate={{
                ...
                <div className="flex min-h-15.5 w-full flex-col items-center gap-1 rounded-xl border border-white/6 bg-white/1.5 p-1.5">
                  <AnimatePresence mode="popLayout">
                    {chain.length === 0 ? (
                      ...
                      chain.map((n, ni) => (
                        <NodeCard key={n.raw} node={n} showConnector={ni > 0} />
                      ))
                    )}
                  </AnimatePresence>
                </div>
